...
const wrTemplate = 'WellRestedIntent how {quality} {verbPhrasing} ' +
...
const sqTemplate = 'SleepQualityIntent {timePeriod} {verbPhrasing} ' +
                  '{-|PreviousNightQuality} {sleepPhrase|}';

const sqInvertedTemplate = 'SleepQualityIntent {verbPhrasing} ' +
                  '{-|PreviousNightQuality} {sleepPhrase|} ' +
                  '{timePeriod|}';

const sampleUtterances = [utterances(wrTemplate, {}, wrDictionary).join('\n'),
                         utterances(sqTemplate, {}, sqDictionary).join('\n'),
                         utterances(sqInvertedTemplate, {}, sqDictionary).join('\n')].join('\n');

fs.writeFile('SampleUtterances.txt', sampleUtterances, err => {
  if(err) {
    throw err;
  }
